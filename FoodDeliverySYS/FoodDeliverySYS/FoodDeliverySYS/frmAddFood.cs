...
namespace FoodDeliverySYS.Models
{
    public partial class frmAddFood : Form
    {
...

        {
            ...
            clbToppings.DataSource = toppings;
            clbToppings.DisplayMember = "ToppingName";
            clbToppings.ValueMember = "ToppingID";
        ...
        private void btnAddPizza_Click(object sender, EventArgs e)
        {
            ...
            if (cmbCrustType.SelectedIndex == -1 || cmbPizzaSize.SelectedIndex == -1)
            {
            ...
            var pizza = new Pizza
            {
                ...
                PizzaName = txtPizzaName.Text.Trim(), 
        ...
        private void RefreshPizzaGrid()
        {
            ...
            dvgPizzaList.DataSource = pizzaList.Select(p => new
            {
                ...
                p.PizzaSize,
                p.CrustType,
                Toppings = string.Join(", ", p.Toppings.Select(t => t.ToppingName)),
                TotalPrice = $"€{p.TotalPrice:N2}"
            }).ToList();
            dvgPizzaList.Columns["PizzaID"].HeaderText = "Pizza ID";
            dvgPizzaList.Columns["PizzaName"].HeaderText = "Pizza Name";
            dvgPizzaList.Columns["PizzaSize"].HeaderText = "Pizza Size";
            dvgPizzaList.Columns["CrustType"].HeaderText = "Crust Type";
            dvgPizzaList.Columns["Toppings"].HeaderText = "Toppings";
            dvgPizzaList.Columns["TotalPrice"].HeaderText = "Total Price";

            dvgPizzaList.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
